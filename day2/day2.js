...
inputList = ["Game 1: 1 green, 2 blue; 15 blue, 12 red, 2 green; 4 red, 6 blue; 10 blue, 8 red; 3 red, 12 blue; 1 green, 12 red, 8 blue",
...
inputList.forEach(item => {
    const [name, roundsData] = item.split(':');
    const rounds = roundsData.split(';').map(round => {
        const colors = round.trim().split(',').map(color => {
            const [quantity, colorName] = color.trim().split(' ');
            return { [colorName]: parseInt(quantity) };
        });
...
const minimalValuesArray = [];


//check each game data
gamesData.forEach(game => {
    ...
    for (let i = 0; i < game.rounds.length; i++) {
        ...
        if (round.red > redCubes || round.green > greenCubes || round.blue > blueCubes) {
            console.log("Round is invalid in", game.name);
            isValidGame = false;
            break; // No need to continue checking rounds once an invalid round is found
        }
    }

    if (isValidGame) {
        console.log("Valid game:", game.name);
        possibleGamesIds.push(game.name)
    } else {
        console.log("Invalid game:", game.name);
    }
    ...
    game.rounds.forEach(round => {
        maxRed = Math.max(maxRed, round.red || -Infinity);
        maxGreen = Math.max(maxGreen, round.green || -Infinity);
        maxBlue = Math.max(maxBlue, round.blue || -Infinity);
    });

    minimalValuesArray.push({
        gameName: game.name,
        highestRed: maxRed === -Infinity ? null : maxRed,
        highestGreen: maxGreen === -Infinity ? null : maxGreen,
        highestBlue: maxBlue === -Infinity ? null : maxBlue
    });
});

console.log(possibleGamesIds);

const gameNumbers = possibleGamesIds.map(name => parseInt(name.match(/\d+/)[0]));
...
console.log(minimalValuesArray);
let multipliedValues = [];

minimalValuesArray.forEach(gameMinimal => {
    multipliedValues.push((gameMinimal.highestRed * gameMinimal.highestBlue * gameMinimal.highestGreen))
});

console.log("All multiplyed values", multipliedValues);
