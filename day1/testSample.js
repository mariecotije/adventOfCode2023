...
["one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "1", "2", "3", "4", "5", "6", "7", "8", "9"].forEach(digit => {
    //find first occurence of matching digit in each string and save the digit value into a variable
    let firstOccurence = inputValues.map(string => string.indexOf(digit));

    //find first occurence of matching digit in each string  from the end and save the digit value into a variable
    let lastOccurence = inputValues.map(string => string.lastIndexOf(digit));

    twoDigits.push(firstOccurence.filter((value, index) => value !== -1 && value === lastOccurence[index]));
})
